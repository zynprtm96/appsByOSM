...
<a href="../kar/laporanpenjualan.php" type="button" class="btn btn-danger">Kembali</a>
    ...
    <br>
    <h5 style="text-align: center;">GRAFIK LAPORAN PENJUALAN</h5>
    ...
    <br>
        ...
        if (isset($_POST['search'])) {
            ...
            $tanggal = mysqli_query($konek,"SELECT tanggal FROM penjualan WHERE nonota LIKE '%".$keyword."%'  ORDER BY nonota DESC");
            $pembayaran = mysqli_query($konek,"SELECT pembayaran FROM penjualan WHERE nonota LIKE '%".$keyword."%'  ORDER BY nonota DESC");
        }elseif (isset($_POST['range'])) {
            $date1 = $_POST['date1'];
            $date2 = $_POST['date2'];
            $tanggal = mysqli_query($konek,"SELECT  tanggal FROM penjualan WHERE tanggal BETWEEN '$date1' and '$date2' group by tanggal ");
            $pembayaran = mysqli_query($konek,"SELECT tanggal, sum(pembayaran) as pembayaran FROM penjualan WHERE tanggal BETWEEN '$date1' and '$date2' group by tanggal ");
            
        }elseif(isset($_POST['reset'])){
            $tanggal = mysqli_query($konek,"SELECT  tanggal FROM penjualan GROUP BY tanggal ");
            $pembayaran = mysqli_query($konek,"SELECT tanggal , sum(pembayaran) as pembayaran FROM penjualan group by tanggal ");
...
<script  type="text/javascript">
  ...
  var data = {
            labels: [<?php while ($p = mysqli_fetch_array($tanggal)) { echo '"' . date('d M Y',strtotime($p['tanggal'])). '",';}?>],
            
            datasets: [
            {
              label: "Uang Masuk",
              borderColor:"#45f248",
              data: [<?php while ($p = mysqli_fetch_array($pembayaran)) { 
