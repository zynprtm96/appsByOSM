...
<table class="table container" style="font-size: 14px;">
    ...
    <tbody>
        ...
        if (isset($_POST['search'])) {
            ...
            $query = mysqli_query($konek,"SELECT * FROM nota_batal WHERE nonota LIKE '%".$keyword."%' OR nama LIKE '%".$keyword."%'  OR nohp  LIKE '%".$keyword."%' ORDER BY nonota DESC");
        }elseif (isset($_POST['range'])) {
            $date1 = $_POST['date1'];
            $date2 = $_POST['date2'];
            $query = mysqli_query($konek,"SELECT * FROM nota_batal WHERE tanggal_batal BETWEEN '$date1' and '$date2' order by id desc");
        }elseif(isset($_POST['reset'])){
            $query = mysqli_query($konek,"SELECT * FROM nota_batal order by id desc");
