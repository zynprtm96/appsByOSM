...
<header class="header">
    ...
    <form method="POST" action="" class="form-inline " >
     ...
     <button class="btn btn-primary btn-sm" style="margin-left:70%;" onclick="printContent('div1')">Cetak Laporan</button>
...
<table class="table container" style="font-size: 14px;">
    ...
    <tbody>
        ...
        if (isset($_POST['search'])) {
            ...
            $query = mysqli_query($konek,"SELECT * FROM penjualan WHERE nonota LIKE '%".$keyword."%'  ORDER BY nonota DESC");
        }elseif (isset($_POST['range'])) {
            $date1 = $_POST['date1'];
            $date2 = $_POST['date2'];
            $query = mysqli_query($konek,"SELECT * FROM penjualan WHERE tanggal BETWEEN '$date1' and '$date2' order by id desc");
        }elseif(isset($_POST['reset'])){
            $query = mysqli_query($konek,"SELECT * FROM penjualan order by id desc");

        }
        else{
            $query = mysqli_query($konek,"SELECT * FROM master_pelanggan order by id desc");
        }
        ...
        if ($hitung_data > 0) {
            ...
            while ($data = mysqli_fetch_array($query)) {
                ...
                <tr align="center"  >
                    <td></td>
                    <td  ><input type="text" id="nama" readonly value="<?php echo $data['nama']; ?>"></td>
                    <td ><?php echo $data['nohp']; ?></td>
                    <td >
                        <div id="notapesan"></div>
                        <div id="notakontan"></div>
                        <div id="rekam"></div>
                    </td>
