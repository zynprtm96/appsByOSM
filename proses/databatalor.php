...
<table class="table  container" style="font-size: 14px;">
    ...
    <tbody>
        ...
        $query = mysqli_query($konek,"SELECT * FROM order_lab WHERE nonotaor LIKE '%".$keyword."%'   ORDER BY nonotaor DESC");
            ...
            while ($data = mysqli_fetch_array($query)) {
              if($data['batal']=="Y"){
                ...
                <tr align="center" <?php echo $batal?> >
                    <td><?php echo date('d M Y',strtotime($data['tanggal_order'])) ?></td>
                    <td ><?php echo $data['nonotaor']; ?></td>
                    <td ><?php echo $data['nota_ref']; ?></td>
                   
                    
                   
                    <td > 
                        <button title="Batalkan Nota" onclick="batal('<?php echo $data['nonotaor'];?>')" class="btn btn-warning"><i style="font-size: 18px;" class='bx bx-calendar-exclamation'></i></button>
  ...
  content: "input",
    ...
    window.location = "../proses/batalor.php?id="+ id + "&alasan="+ value + "&nota=<?php echo $nb?>";
